...
LIMIT_METRICS = {"upper_limit_fwd", "upper_limit_rev"}


def _draw_tieline_ax(ax, pair_df, color_map, label_map, tieline_metrics):
    """Draw tieline sparklines on a single axes."""
    dt = pair_df["datetime"]

    # Shaded area under reserved metrics
    for m in ("reserved_fwd", "reserved_rev"):
        if m not in tieline_metrics or m not in pair_df.columns:
            continue
        vals = pair_df[m].values
        c = color_map.get(m, GREY)
        ax.fill_between(dt, 0, vals, step="post", color=c, alpha=0.18)

    for m in tieline_metrics:
        if m not in pair_df.columns:
            continue
        vals = pair_df[m]
        if vals.isna().all():
            continue
        c = color_map.get(m, GREY)
        is_limit = m in LIMIT_METRICS
        ax.step(dt, vals, where="post",
                color=c, linewidth=0.7 if is_limit else 0.9,
                linestyle="--" if is_limit else "-",
...
def generate_tieline_pdf(
    tieline_data: dict[str, pd.DataFrame],
    pairs: list[str],
    tieline_metrics: list[str],
    color_map: dict[str, str],
    label_map: dict[str, str],
    view_start: date,
    view_end: date,
    ...
    """
    Build a multi-page A4 PDF for tieline (interconnector) data.

    One page per market (DCM / DAM), with one row per interconnector pair.

    Parameters
    ----------
    tieline_data : dict mapping market label → wide-format DataFrame
        (already filtered to the visible date range).
    pairs : list of interconnector pair names in display order.
    tieline_metrics : metric keys to plot.
    color_map / label_map : styling dicts.
    view_start, view_end : date range shown in title.
    ...
    with PdfPages(buf) as pdf:
        for market_label, df in tieline_data.items():
            if df.empty:
                continue

            active_pairs = [p for p in pairs if p in df["pair"].unique()]
            n_pairs = len(active_pairs) or 1
            ...
            fig.text(
                0.5, 1 - 0.28 / A4_H,
                f"Interconnectors — {market_label}  —  {date_label}",
                ha="center", va="top",
                fontsize=11, fontweight="bold", color=NAVY,
            )
            ...
            for i, pair in enumerate(active_pairs):
                ax = axes[i, 0]
                rdf = df[df["pair"] == pair].sort_values("datetime")
                ...
                if rdf.empty:
                    ...
                    ax.set_yticks([])
                    ax.set_xticks([])
                    ax.set_ylabel(pair, fontsize=6, fontweight="bold",
                                  color=NAVY, labelpad=8)
                    continue

                _draw_tieline_ax(ax, rdf, color_map, label_map, tieline_metrics)
                ax.set_ylabel(pair, fontsize=6, fontweight="bold",
                              color=NAVY, labelpad=8)
            ...
            fig.legend(
                handles, labels,
                loc="lower center",
                ncol=min(len(tieline_metrics), 4),
                fontsize=5.5,
                frameon=False,
                bbox_to_anchor=(0.5, 0.01),
            )
