...
| F1–F5 | Frontend items | P2 | Mostly fixed — F5 fixed (`review/2026-09-25`): http(s)-only links at ingest, export and `openUrl`. F2 fixed: create-vite tsconfig layout (nothing emitted) and `--config vite.config.ts` in every script. F1 fixed for the cited cases: app-wide notice when the export is missing, no invented Deep Dive run line or demand change, MarketData KPIs from the areas that loaded, failed areas tagged; fixtures remain the silent loading state elsewhere. F3 fixed: `lib/time.ts` (DB timestamp, day, wall clock) with vitest tests under two timezones; the duplicated parsers use it. F4 partial: `s()` caches and freezes its parse; splitting `MarketData.tsx` and moving the 66 raw `onClick` elements to `Hoverable` remain (needs design review) |
| T1–T8 | Tooling, tests, docs items | P2 | Partial — T4 fixed except the `conftest.py` (58 identical two-line setups; moving them is churn with no behaviour gain): `hf_sync` and web-api guard tests, a stubbed `run-all` smoke test, no hardcoded committee count; T8 fixed (`review/2026-09-25`): webhook URL kept out of logs (also httpx's request log), d3 pinned with SRI. No busy-timeout change: Python's `sqlite3` already waits 5 s and no lock errors are recorded. T5 fixed: one gate over every tracked path and file. T7 fixed: Space sync mirrors deletions. T6 fixed: actions SHA-pinned, `permissions: contents: read` everywhere, Dependabot for pip/npm/actions, and every Python install pinned by `constraints.txt`. T3 fixed: CI `web` job runs ESLint (hook rules), the build and vitest. T1 fixed: each GOTCHAS `(open` entry checked against the code; stale ones rewritten (date sweep, committee duplicate, `hf_sync` filename, notebook leak, legacy code, iframe audit, ruff, Docker, OCCTO probe, frozen captions). T2 fixed: mypy (3.12 syntax for numpy's stubs) gates every module in CI; the models moved to `Mapped[]` (schema byte-identical), and scraped links go through a typed `href_of` |
