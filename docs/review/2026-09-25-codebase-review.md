...
| F1–F5 | Frontend items | P2 | Partial — F5 fixed (`review/2026-09-25`): http(s)-only links at ingest, export and `openUrl`. F2 fixed: create-vite tsconfig layout (nothing emitted) and `--config vite.config.ts` in every script. F1 fixed for the cited cases: app-wide notice when the export is missing, no invented Deep Dive run line or demand change, MarketData KPIs from the areas that loaded, failed areas tagged; fixtures remain the silent loading state elsewhere. F3 fixed: `lib/time.ts` (DB timestamp, day, wall clock) with vitest tests under two timezones; the duplicated parsers use it |
