"""Unit tests for the period-comparison stats helpers in dashboard.read.

Uses a temporary SQLite path via pytest's ``tmp_path`` with synthetic rows.
No network, no Streamlit runtime. Mirrors ``tests/test_eprx_upsert.py`` style.
"""
...
def _bal_row(metric: str, value: float, time: str, day: int) -> EprxBalancing:
    return EprxBalancing(
        product_code="1-0", product="Primary", area="tepco",
        date=dt.date(2026, 4, day), time=time, block_num=1, blocks_per_day=48,
        metric=metric, value=value, jfy=2026, source_file="f.csv",
    )


def _seed_balancing(db_path: str) -> None:
    """Primary/tepco: demand 100/200, contracted 40/60, price_avg 5/15,
    price_max 8/12 across two slots."""
    session = get_session(db_path)
    try:
        session.add_all([
...
def test_balancing_period_stats(tmp_path):
    db_path = str(tmp_path / "t.db")
    init_db(db_path)
    _seed_balancing(db_path)

    s = balancing_period_stats("Primary", "tepco",
                               dt.date(2026, 4, 1), dt.date(2026, 4, 1), db_path)
    assert s["avg_demand_mw"] == 150.0          # (100+200)/2
...
def test_balancing_period_stats_empty(tmp_path):
    ...
    s = balancing_period_stats("Secondary 1", "tepco",
                               dt.date(2026, 4, 1), dt.date(2026, 4, 1), db_path)
    assert s["avg_demand_mw"] is None
    assert s["avg_contracted_mw"] is None
    assert s["avg_unprocured_mw"] is None
    assert s["avg_price"] is None
    assert s["avg_max_price"] is None
